...
func (tc *TimeCache) GetVideoFromDB(videoname string) (*metadata.Metadata, error) {
	filter := bson.D{primitive.E{Key: "videoname", Value: videoname}}

	var result metadata.Metadata
	err := tc.Client.Database("Hypeman").Collection("Metadata").FindOne(context.TODO(), filter).Decode(&result)

	if err != nil {
		return nil, err
	}

	database := tc.Client.Database("Hypeman")

	filter = bson.D{{"videoname", videoname}}

	//Manual Calculation of the Ratings data
